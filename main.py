...
def start(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(start_message(first_name))
...
def handle_media(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(start_message(user_first_name))


def new_member(update: Update, context: CallbackContext):
    new_member = update.message.new_chat_members[0]
    update.message.reply_text(
        f"👋🏻 Welcome to the group, {new_member.first_name}! Feel free to ask me anything."
    )


def left_member(update: Update, context: CallbackContext):
    left_member = update.message.left_chat_member
    update.message.reply_text(f"❌ {left_member.first_name} left the group.")
...
def main():
    ...
    dp.add_handler(
        MessageHandler(
            Filters.photo | Filters.document | Filters.audio | Filters.video,
            handle_media))
